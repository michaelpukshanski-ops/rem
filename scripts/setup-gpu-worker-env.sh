...
echo "🔧 Setting up GPU Worker .env file"
echo "===================================="
echo ""
...
if [ ! -f "terraform.tfstate" ]; then
  echo "❌ Error: Terraform state not found. Have you run 'terraform apply'?"
  exit 1
fi
...
if [ -z "$SQS_QUEUE_URL" ] || [ -z "$RAW_AUDIO_BUCKET" ] || [ -z "$TRANSCRIPTS_BUCKET" ] || [ -z "$DYNAMODB_TABLE" ]; then
  echo "❌ Error: Could not read all required Terraform outputs"
